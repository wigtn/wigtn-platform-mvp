...
test("질문 등록 후 AI 첫 답변 상태 전이가 완료된다", async ({ page }) => {
  ...
  await expect(page.getByText("질문 안전 검사")).toBeVisible();
  await expect(
    page.getByText("예산이 아니라 ‘결정 조건’을 먼저 물어보세요."),
...
test("프로필 수정과 검증 배지 신청 상태가 저장된다", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "합성 증빙으로 신청 체험" }).click();
...
test("관리자 역할로 리뷰 블라인드와 복구를 체험한다", async ({ page }) => {
  ...
  const blind = page
    .locator(".admin-row")
    .filter({ hasText: "목표와 지원 체계가 같은 방향을 봅니다" })
...
test("관리자가 회사 수기 등록과 콘텐츠 블라인드를 실행한다", async ({
  ...
  await page.getByRole("button", { name: "후보 수집 dry-run" }).click();
