...
function QuestionForm({
  ...
  // AI 호출은 lib/demo-ai.ts 를 쓴다(현상 구현). 큐에 넣고 워커가 실제
  // OpenAI 를 부르는 경로라, 화면은 진행 상태만 받아 표시한다.
  //
  // 화면을 떠나면 진행 중인 요청을 끊는다. 안 끊으면 사라진 화면에
  // setState 가 걸린다.
  ...
  const ask = async (event: FormEvent) => {
    ...
    // 질문 글은 내 액션 원장에도 남긴다. 새로고침해도 남아 있어야 한다.
